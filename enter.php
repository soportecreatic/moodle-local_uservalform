...
// Course ID is required to use the validation form.
// We receive a error parameter in case that the user doesn't enter a correct value.
$courseid = required_param('courseid', PARAM_INT);
$validationerror = optional_param('error', '', PARAM_INT);

// Defining return and course view URLs.
$return = new moodle_url('/');
$courselink = new moodle_url('/course/view.php', array('id' => $courseid));

// Gets course data from the database, and defines a variable with a course context.
...
// We get DB records about user custom fields and validation form data.
...
// Generate a new validation form.
...
// Process data submitted from validation form.
    ...
    if($data->validationinfo == $userinfodata->data) {
        ...
        // If last access data exists.
        if($lastaccess) {
            // If 'show only once' option is disabled, updates sesskey data
            // in last access information and sets checked to false.
            if($uservalformdata->showonlyonce == false) {
                $lastaccess->sesskey = $USER->sesskey;
                $lastaccess->checked = false;
            } else {
                // Otherwise, erases sesskey sets checked to true.
                $lastaccess->checked = true;
                $lastaccess->sesskey = null;
            }
            // And update last access information in the database.
